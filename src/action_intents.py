...
_CONTACT_DETAIL = r"(?:phone(?:\s+number)?|mobile|cell(?:\s+phone)?|email(?:\s+address)?|address|contact\s+(?:info|details|information))"
_CONTACT_BOOK = r"(?:contacts?|address\s+book|vcards?)"
...
_ROUTING_PATTERNS: tuple[tuple[str, str, Pattern[str]], ...] = tuple(
    ...
    for category, reason, pattern in (
        ...
        # Contact lookups. Plain chat cannot know stored phone numbers,
        # addresses, or contact-card details, so route these to the agent while
        # still letting explanatory "how do I..." questions remain chat.
        ("contacts", "contact lookup request", rf"\b(?:find|look\s+up|lookup|show|search|open)\b.{{0,120}}\b(?:my\s+|the\s+)?{_CONTACT_BOOK}\b"),
        ("contacts", "contact detail request", rf"\b(?:what(?:'s| is)|find|look\s+up|lookup|show|give\s+me|get|tell\s+me)\b.{{0,120}}\b\w+['’]s\s+{_CONTACT_DETAIL}\b"),
        ("contacts", "contact detail request", rf"\b(?:what(?:'s| is)|find|look\s+up|lookup|show|give\s+me|get|tell\s+me)\b.{{0,120}}\b{_CONTACT_DETAIL}\b.{{0,80}}\b(?:for|of)\s+\w+\b"),
        ("contacts", "contact detail request", rf"\b(?:what(?:'s| is)|find|look\s+up|lookup|show|give\s+me|get|tell\s+me)\b.{{0,120}}\b\w+\b.{{0,40}}\bcontact\s+(?:info|details|information)\b"),
        ("contacts", "contact detail request", rf"\b{_CONTACT_DETAIL}\b.{{0,80}}\b(?:for|of)\s+\w+\b"),
        ("contacts", "contact phone lookup request", r"\b(?:mi\s+(?:dici|trovi|cerchi)|dimmi|trova|cerca)\b.{0,120}\b(?:numero|telefono|cellulare|mail|email|indirizzo)\b"),
        ("contacts", "contact phone lookup request", r"\b(?:numero|telefono|cellulare|mail|email|indirizzo)\b.{0,80}\b(?:di|del|della|dei|degli|delle)\s+\w+\b"),
