...
- AFTER A TOOL SUCCEEDS, do not second-guess ordinary successful results. For file mutations, only claim success when `write_file` or `edit_file` returned success/exit_code 0 for the target file; shell stdout or prose is not proof of a file change. Reply in ONE short sentence confirming what was done.
...
- If a workspace is active, call `get_workspace` when the target path is implicit, and use workspace-relative paths with `ls`, `read_file`, `write_file`, or `edit_file`.
...
def _is_ollama_openai_compat_url(endpoint_url: str) -> bool:
    ...
    are present, so use the model/tool heuristics unless the endpoint explicitly
    opts in or out via ModelEndpoint.supports_tools.
    """
    try:
...
def _should_use_native_tools(
    endpoint_url: str,
    endpoint_supports: Optional[bool],
    model_supports_tools: bool,
    model_no_tools: bool,
    is_ollama_native: bool,
    is_ollama_openai_compat: bool,
) -> bool:
    if endpoint_supports is True:
        return True
    if endpoint_supports is False or model_no_tools or is_ollama_native:
        return False
    if is_ollama_openai_compat:
        return model_supports_tools
...
def _compute_final_metrics(
    ...
    if provider_cost and provider_cost > 0:
        metrics["provider_cost"] = round(provider_cost, 8)
    if prep_timings:
...
async def _cancel_running_tool_task(task: "asyncio.Task", tool_name: str) -> None:
    """Cancel an in-flight tool task when the SSE client disconnects."""
    if task.done():
        return
    task.cancel()
    with contextlib.suppress(asyncio.CancelledError):
        await task
    logger.info("Cancelled in-flight tool task after stream disconnect: %s", tool_name)
...
async def stream_agent_loop(
    ...
    if _direct_low_signal:
        ...
        try:
            async for chunk in stream_llm_with_fallback(
                ...
                prompt_type=None,
                tools=None,
                timeout=_int_setting("agent_stream_timeout_seconds", 300),
                ...
                if chunk.startswith("data: ") and not chunk.startswith("data: [DONE]"):
                    ...
                    if data.get("type") == "usage":
                        ...
                        if usage.get("provider_cost") is not None:
                            try:
                                direct_provider_cost += float(usage.get("provider_cost") or 0)
                            except (TypeError, ValueError):
                                pass
                        continue
        ...
        if direct_provider_cost > 0:
            metrics["provider_cost"] = round(direct_provider_cost, 8)
    ...
    _is_api_model = _should_use_native_tools(
        endpoint_url or "",
        _endpoint_supports,
        _model_supports_tools,
        _model_no_tools,
        _is_ollama_native,
        _ollama_openai_compat,
    )
    ...
    try:
        ...
        soft_budget = _int_setting("agent_input_token_budget", DEFAULT_BUDGET)
        if soft_budget > 0:
    ...
    for round_num in range(1, max_rounds + 1):
        ...
        agent_stream_timeout = _int_setting("agent_stream_timeout_seconds", 300)
                ...
                try:
                    ...
                    if data.get("type") == "tool_call_delta":
                        ...
                        if u.get("provider_cost") is not None:
                            try:
                                provider_cost += float(u.get("provider_cost") or 0)
                            except (TypeError, ValueError):
                                pass
        ...
        for i, block in enumerate(tool_blocks):
            ...
            if tool_policy and tool_policy.blocks(block.tool_type):
                ...
                # next event OR the tool finishes (sentinel = None).
                    ...
                    await _cancel_running_tool_task(_tool_task, block.tool_type)
                    raise
