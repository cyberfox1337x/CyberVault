...
def _provider_response_cost(headers) -> Optional[float]:
    """Return a trusted upstream response cost when the provider supplies one."""
    if not headers:
        return None
    raw = None
    for name in ("x-litellm-response-cost", "x-litellm-cost", "x-provider-response-cost"):
        try:
        ...
        try:
            ...
            async with client.stream('POST', target_url, json=payload, headers=h, timeout=stream_timeout) as r:
                ...
                if r.status_code != 200:
                    ...
                    return
                provider_cost = _provider_response_cost(r.headers)
                    ...
                    if evt == "response.output_text.delta":
                        ...
                        if input_tokens or output_tokens:
                            ...
                            if provider_cost is not None:
                                usage_data["provider_cost"] = provider_cost
                    ...
                    try:
                        ...
                        if evt == "content_block_start":
                            ...
                            if _anth_input_tokens or _anth_output_tokens:
                                usage_data = {"input_tokens": _anth_input_tokens, "output_tokens": _anth_output_tokens}
                                if provider_cost is not None:
                                    usage_data["provider_cost"] = provider_cost
                                yield f'data: {json.dumps({"type": "usage", "data": usage_data})}\n\n'
                            yield "data: [DONE]\n\n"
                        ...
                        if data.strip():
                            if data.startswith("{"):
                                ...
                                if "usage" in j and not _delta_has_output:
                                    ...
                                    if provider_cost is not None:
                                        _usage_data["provider_cost"] = provider_cost
