...
def research_json_path(session_id: str, root: Optional[Path] = None) -> Optional[Path]:
    """Resolve a persisted research JSON path under the storage root."""
    if not isinstance(session_id, str) or not _RESEARCH_SESSION_ID_RE.fullmatch(session_id):
        return None
    root_path = Path(root if root is not None else DEEP_RESEARCH_DIR).resolve()
    path = (root_path / f"{session_id}.json").resolve()
    try:
