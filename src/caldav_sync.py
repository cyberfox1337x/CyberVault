...
def google_caldav_basic_auth_error(url: str) -> str:
    """Return a user-facing error when a Google URL cannot use Basic auth.

    Google's current ``apidata.googleusercontent.com/caldav/v2`` endpoint
    rejects Basic/App Password authentication with HTTP 401. CyberVault's
    generic CalDAV connector is password-based, so fail early with a precise
    explanation instead of blaming the user's username or app password.
    """
    parts = urlparse(url if isinstance(url, str) else "")
    host = (parts.hostname or "").lower()
    path = parts.path.rstrip("/")
    if host != "apidata.googleusercontent.com" or not path.startswith("/caldav/v2"):
        return ""
    segments = [segment for segment in path.split("/") if segment]
    calendar_id = segments[2] if len(segments) >= 3 else ""
    if not calendar_id:
        return (
            "Google CalDAV URL must include a calendar ID, for example "
            "https://apidata.googleusercontent.com/caldav/v2/you@example.com/events. "
            "Google also requires OAuth 2.0 there; app passwords/Basic Auth return HTTP 401."
        )
    return (
        "Google CalDAV at apidata.googleusercontent.com requires OAuth 2.0. "
        "App passwords/Basic Auth return HTTP 401, so CyberVault's password-based "
        "CalDAV connector cannot use that Google endpoint yet."
    )
