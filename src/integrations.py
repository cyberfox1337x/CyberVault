...
def _uses_exact_integration_base_url(integration: Dict[str, Any]) -> bool:
    preset = str(integration.get("preset") or "").strip().lower()
    name = str(integration.get("name") or "").strip().lower()
    return preset == "discord_webhook" or name == "discord webhook"
...
async def execute_api_call(
    ...
    if _uses_exact_integration_base_url(integration):
        if path not in ("", "/"):
            return {
                "error": "Discord Webhook integrations use the exact configured Base URL; path must be /",
                "exit_code": 1,
            }
        url = base_url
    else:
