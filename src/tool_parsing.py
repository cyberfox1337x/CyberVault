...
# Some local fenced-tool models collapse a simple line-arg tool into one line:
#   ```read_file path/to/file.txt```
# The main fenced regex intentionally only accepts JSON on the tag line so
# Markdown info strings such as ```python title="demo.py" stay inert. This
# second pass is deliberately narrower: only a single-line non-JSON inline
# argument is considered, and _inline_arg_tool_call below whitelists which
# tools can safely use that shape.
_INLINE_ARG_TOOL_FENCE_RE = re.compile(
    r"```(" + "|".join(TOOL_TAGS) + r")(?![\w-])"
    r"[ \t]+(?![{\[])([^\r\n`]+?)```",
    re.IGNORECASE,
)
...
def _inline_arg_tool_call(m) -> Optional[ToolBlock]:
    """Recover one-line inline-arg fences that are safe without JSON.

    Keep this intentionally small. ``read_file`` has one primary argument and
    existing path confinement lives in the tool handler, so the shorthand is
    unambiguous. Mutation tools such as ``edit_file`` still require JSON or the
    normal multiline body so we never guess old/new replacement strings.
    """
    tag = m.group(1).lower()
    arg = (m.group(2) or "").strip()
    if not arg or tag in _CODE_FENCE_TAGS:
        return None
    if tag != "read_file":
        return None
    ...
    if not arg:
        return None
    return ToolBlock("read_file", arg)


def _strip_executed_inline_arg_fence(m) -> str:
    """re.sub callback: strip one-line inline fences only when executable."""
    return "" if _inline_arg_tool_call(m) is not None else m.group(0)
...
def parse_tool_blocks(text: str, skip_fenced: bool = False) -> List[ToolBlock]:
    ...
    if not skip_fenced:
        ...
        for m in _INLINE_ARG_TOOL_FENCE_RE.finditer(text):
            block = _inline_arg_tool_call(m)
            if block:
                blocks.append(block)
...
def strip_tool_blocks(text: str, skip_fenced: bool = False) -> str:
    ...
    if not skip_fenced:
        cleaned = _INLINE_ARG_TOOL_FENCE_RE.sub(_strip_executed_inline_arg_fence, cleaned)
