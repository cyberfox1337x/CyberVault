...
async def _delete_endpoint_for_task(task: dict) -> None:
    ...
    elif "ollama" in cmd.lower():
        try:
            from routes.cookbook_helpers import _ollama_bind_from_cmd
            _, ollama_port = _ollama_bind_from_cmd(cmd)
            port = int(ollama_port)
        except Exception:
            logger.debug("cookbook_serve_lifecycle: failed to parse OLLAMA_HOST from scheduled serve", exc_info=True)
            port = 11434
    else:
        port = 8080
