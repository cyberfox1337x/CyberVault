...
from core.auth import AuthManager, TOKEN_TTL, normalize_known_username
...
_login_form_limiter = RateLimiter(max_requests=15, window_seconds=60)
if LOCALHOST_BYPASS:
...
@app.get("/theme")
async def serve_theme(request: Request):
    return await serve_index(request)
...
@app.post("/login")
async def login_form_fallback(request: Request):
    """Native form fallback for login when client JavaScript is blocked.

    The enhanced login page submits JSON to /api/auth/login. If a CDN or CSP
    interaction prevents that script from running, the browser still posts the
    form here instead of falling back to GET /login?... with the password in
    the URL.
    """
    if not AUTH_ENABLED:
        return RedirectResponse(url="/", status_code=303)
    client_host = request.client.host if request.client else "unknown"
    if not _login_form_limiter.check(client_host):
    ...
    remember = (form.get("remember", ["on"])[-1] or "").lower() not in {"", "0", "false", "off", "no"}
    totp_code = (form.get("totp_code", [""])[0] or "").strip()

    redirect = RedirectResponse(url="/login?error=invalid", status_code=303)
    if not auth_manager.is_configured:
        return redirect
    if not await asyncio.to_thread(auth_manager.verify_password, username, password):
        return redirect
    if auth_manager.totp_enabled(username):
        if not totp_code or not auth_manager.totp_verify(username, totp_code):
            return RedirectResponse(url="/login?error=totp", status_code=303)
