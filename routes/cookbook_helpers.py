...
def _append_llama_cpp_linux_accel_build_lines(runner_lines: list[str]) -> None:
    ...
    runner_lines.append('      return 1')
    runner_lines.append('    }')
    runner_lines.append('    _odysseus_has_vulkan_build_deps() {')
    runner_lines.append('      command -v glslc >/dev/null 2>&1 || return 1')
    runner_lines.append('      [ -e /usr/include/vulkan/vulkan.h ] || [ -e /usr/local/include/vulkan/vulkan.h ] || return 1')
    runner_lines.append('      ldconfig -p 2>/dev/null | grep -q \'libvulkan\\.so \' && return 0')
    runner_lines.append('      [ -e /usr/lib/libvulkan.so ] && return 0')
    runner_lines.append('      [ -e /usr/lib/x86_64-linux-gnu/libvulkan.so ] && return 0')
    runner_lines.append('      [ -e /usr/local/lib/libvulkan.so ] && return 0')
    runner_lines.append('      return 1')
    runner_lines.append('    }')
    ...
    runner_lines.append('      fi')
    runner_lines.append('    elif _odysseus_has_vulkan_device && _odysseus_has_vulkan && _odysseus_has_vulkan_build_deps; then')
    ...
    runner_lines.append('    else')
    runner_lines.append('      if _odysseus_has_vulkan_device && _odysseus_has_vulkan && ! _odysseus_has_vulkan_build_deps; then')
    runner_lines.append('        echo "[odysseus] WARNING: Vulkan runtime/device found, but build deps are missing (need glslc, vulkan.h, and libvulkan.so) — building llama-server for CPU only."')
    runner_lines.append('      else')
    runner_lines.append('        echo "[odysseus] WARNING: no HIP/CUDA/Vulkan toolchain found — building llama-server for CPU only."')
    runner_lines.append('      fi')
    runner_lines.append('      echo "[odysseus]   GPU inference will not be available for this llama.cpp build."')
    runner_lines.append('      echo "[odysseus]   Install Vulkan SDK/dev packages (glslc, vulkan.h, libvulkan.so) / ROCm for AMD GPUs or CUDA tooling for NVIDIA, then re-launch this serve task."')
    runner_lines.append('      rm -rf build')
    runner_lines.append('      cmake -B build -DCMAKE_BUILD_TYPE=Release && cmake --build build -j"$NPROC" --target llama-server && ln -sf ~/llama.cpp/build/bin/llama-server ~/bin/llama-server')
    runner_lines.append('    fi')
