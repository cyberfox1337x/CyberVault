...
async def build_chat_context(
    ...
    group_character_name: str | None = None,
) -> ChatContext:
    ...
    if group_system_prompt:
        prompt = str(group_system_prompt).strip()
        if prompt:
            name = str(group_character_name or "").strip()
            name_line = f"Your name is {name}." if name else ""
            if name_line and name_line not in prompt:
                prompt = f"{name_line}\n\n{prompt}"
            preface.insert(0, {"role": "system", "content": prompt})
