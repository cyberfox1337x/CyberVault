...
def setup_research_routes(research_handler, session_manager=None) -> APIRouter:
    ...
    def _trusted_research_json_entries() -> list[Path]:
        data_dir = Path(DEEP_RESEARCH_DIR).resolve()
        trusted = []
        try:
    ...
    def _find_research_json_path(session_id: str) -> Optional[Path]:
        """Find a persisted research JSON by trusted directory enumeration.

        Keep the route-level path flow visible to static analyzers: validate
        the route parameter first, enumerate files from the configured storage
        root, match on the trusted entry filename, then resolve and confine the
        matched path before any read/write/delete operation.
        """
        _validate_session_id(session_id)
        for candidate in _trusted_research_json_entries():
            if candidate.suffix == ".json" and candidate.stem == session_id:
                return candidate
        return None

    def _result_json_path(session_id: str) -> Path:
        path = _find_research_json_path(session_id)
        if path is None:
            raise HTTPException(404, "Research not found")
        return path

    def _active_research_owned(session_id: str, user: str) -> bool:
        entry = research_handler._active_tasks.get(session_id)
        return entry is not None and entry.get("owner", "") == user

    def _owned_research_json_path(session_id: str, user: str) -> Optional[Path]:
        path = _find_research_json_path(session_id)
        if path is None or not path.exists():
            return None
        try:
            if json.loads(path.read_text(encoding="utf-8")).get("owner") == user:
                return path
        except Exception:
            return None
        return None

    def _require_owned_research_path(session_id: str, user: str) -> Path:
        path = _owned_research_json_path(session_id, user)
        if path is None:
            raise HTTPException(404, "Research not found")
        return path
    ...
    def _owns_in_memory(session_id: str, user: str) -> bool:
        ...
        return _active_research_owned(session_id, user) or _owned_research_json_path(session_id, user) is not None
    ...
    async def research_library(
        ...
        items = []
        for p in _trusted_research_json_entries():
            try:
    ...
    async def research_detail(session_id: str, request: Request):
        ...
        user = _require_user(request)
        _validate_session_id(session_id)
        path = _assert_owns_research(session_id, user)
        try:
    ...
    async def research_delete(session_id: str, request: Request):
        ...
        user = _require_user(request)
        _validate_session_id(session_id)
        json_path = _assert_owns_research(session_id, user)
        deleted = False
        if json_path.exists():
            json_path.unlink()
            deleted = True
    ...
    async def research_result_peek(session_id: str, request: Request):
        ...
        _validate_session_id(session_id)
        disk_path = None
        if not _active_research_owned(session_id, user):
            disk_path = _require_owned_research_path(session_id, user)
        result = research_handler.get_result(session_id)
        if result is None:
            p = disk_path or _require_owned_research_path(session_id, user)
            if p.exists():
                d = json.loads(p.read_text(encoding="utf-8"))
                return {
                    "result": d.get("result", ""),
    ...
    async def research_spinoff(session_id: str, request: Request):
        ...
        path = disk_path or _require_owned_research_path(session_id, user)
        if path.exists():
            try:
