...
_GGUF_QUANT_TOKEN_RE = re.compile(r"(?:I?Q[2-8](?:_[A-Za-z0-9]+){0,3}|UD-[A-Za-z0-9_]+)", re.I)
_GGUF_DISPLAY_QUANT_RE = re.compile(r"(?:QAT[-_]?INT[48]|INT[48]|[48]BIT|NF4|FP4)", re.I)


def _case_flexible_glob_token(token: str) -> str:
    return "".join(f"[{ch.lower()}{ch.upper()}]" if ch.isalpha() else ch for ch in token)


def _normalize_gguf_download_include(repo_id: str, include: str | None) -> str | None:
    """Keep GGUF include filters from matching zero files for label-only quants.

    HuggingFace include patterns are case-sensitive. Catalog/display labels can
    be `QAT-INT4`, while the actual GGUF file is named with a token like
    `q4_0`. Preserve explicit user patterns for non-GGUF repos, make real GGUF
    quant tokens case-flexible, and fall back to `.gguf` files when the include
    only carries a display quant label that cannot match filenames directly.
    """
    if not include or "gguf" not in (repo_id or "").lower():
        return include
    if _GGUF_QUANT_TOKEN_RE.search(include):
        return _GGUF_QUANT_TOKEN_RE.sub(
            lambda m: _case_flexible_glob_token(m.group(0)),
            include,
        )
    if _GGUF_DISPLAY_QUANT_RE.search(include):
        return "*.gguf"
    return include
...
def setup_cookbook_routes() -> APIRouter:
    ...
    async def model_download(request: Request, req: ModelDownloadRequest):
        ...
        if is_ollama_download:
            ...
            req.include = _normalize_gguf_download_include(req.repo_id, _validate_include(req.include))
        ...
        _dl_pyarg = f", allow_patterns={json.dumps(req.include)}" if req.include else ""
        ...
        for task in tasks:
            ...
            if is_alive or (local_win_task and full_snapshot):
                ...
                if has_exit and task_type == "serve":
                    ...
                    if download_has_ok and re.search(r"Fetching\s+0\s+files", full_snapshot, re.IGNORECASE):
                        status = "error"
                        download_zero_files = True
                    elif download_has_failed:
                        status = "error"
                    # Dependency installs are tracked as download tasks but only
                    # emit the generic runner exit marker, not HF download markers.
                    elif download_has_incomplete_evidence and not download_has_ok:
