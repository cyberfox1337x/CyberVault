...
def setup_chat_routes(
    ...
    async def chat_stream(request: Request) -> StreamingResponse:
        ...
        group_system_prompt = (form_data.get("group_system_prompt") or "").strip()
        group_character_name = (form_data.get("group_character_name") or "").strip()
        ...
        try:
            ...
            if group_system_prompt:
                if not (getattr(sess, "name", "") or "").startswith("[GRP]"):
                    group_system_prompt = ""
                    group_character_name = ""
                else:
                    group_system_prompt = group_system_prompt[:10000]
                    group_character_name = group_character_name[:120]
            if _clear_orphaned_session_endpoint(sess, owner=owner):
                raise HTTPException(400, "Selected model endpoint was removed. Pick another model in Settings.")
        ...
        ctx = await build_chat_context(
            ...
            group_character_name=group_character_name,
        )
