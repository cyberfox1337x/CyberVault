...
def setup_calendar_routes() -> APIRouter:
    ...
    async def save_config(request: Request):
        ...
        from src.caldav_sync import google_caldav_basic_auth_error, validate_caldav_url
        try:
            validated_url = validate_caldav_url(body.get("url", ""))
        except ValueError as e:
            raise HTTPException(400, str(e))
        google_error = google_caldav_basic_auth_error(validated_url)
        if google_error:
    ...
    async def add_caldav_account(request: Request):
        ...
        try:
            ...
            raise HTTPException(400, str(e))
        google_error = google_caldav_basic_auth_error(url)
        if google_error:
            raise HTTPException(400, google_error)
    ...
    async def update_caldav_account(account_id: str, request: Request):
        ...
        if body.get("url"):
            ...
            try:
                ...
                raise HTTPException(400, str(e))
            google_error = google_caldav_basic_auth_error(acc["url"])
            if google_error:
                raise HTTPException(400, google_error)
        ...
        if body.get("username") is not None:
            acc["username"] = (body.get("username") or "").strip()
        if body.get("password"):
            from src.caldav_sync import google_caldav_basic_auth_error
            google_error = google_caldav_basic_auth_error(acc.get("url", ""))
            if google_error:
                raise HTTPException(400, google_error)
            from src.secret_storage import encrypt
            acc["password"] = encrypt(body["password"])
