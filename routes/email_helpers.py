        ...
        try:
            ...
            if row.owner:
                if row.owner != owner:
                    ...
                    raise HTTPException(404, "Account not found")
            elif (row.imap_user or "") != owner and (row.from_address or "") != owner:
                # Owner-less rows are legacy single-user data. In authenticated
                # mode only let the matching mailbox owner keep using them; do
                # not let any signed-in user mutate arbitrary legacy accounts.
                raise HTTPException(404, "Account not found")
        finally:
            db.close()
