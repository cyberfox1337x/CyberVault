...
def _write_email_summary_cache(
    message_id: str,
    account_owner: str,
    uid,
    folder: str,
...
def _scheduled_poll_once() -> dict:
    ...
    try:
        ...
        for r in rows:
            ...
            claimed = False
            try:
                conn_claim = sqlite3.connect(SCHEDULED_DB)
                try:
                    cur = conn_claim.execute(
                        "UPDATE scheduled_emails SET status='sending', error=NULL WHERE id=? AND status='pending'",
                        (sid,),
                    )
                ...
                conn2.execute("UPDATE scheduled_emails SET status='sent' WHERE id=? AND status='sending'", (sid,))
                conn2.commit()
                conn2.close()
                ...
                if claimed:
                    conn2 = sqlite3.connect(SCHEDULED_DB)
                    conn2.execute("UPDATE scheduled_emails SET status='failed', error=? WHERE id=? AND status='sending'", (str(e), sid))
                    conn2.commit()
                    conn2.close()
