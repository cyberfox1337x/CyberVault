...
def setup_email_routes():
    ...
    def _email_account_scope_for_owner(query, EmailAccount, owner: str):
        """Return the same account set list/config expose to this owner."""
        if not owner:
            return query
        from sqlalchemy import and_, or_
        ...
        return query.filter(or_(EmailAccount.owner == owner, and_(unowned, same_mailbox)))
    ...
    async def update_email_config(data: dict, owner: str = Depends(require_owner)):
        ...
        try:
            ...
            q = _email_account_scope_for_owner(q, EmailAccount, owner)
            row = q.first()
            if row is None:
            ...
            clear_q = _email_account_scope_for_owner(clear_q, EmailAccount, owner)
    ...
    async def create_email_account(data: dict, owner: str = Depends(require_owner)):
        ...
        try:
            ...
            scope_q = _email_account_scope_for_owner(scope_q, EmailAccount, owner)
    ...
    async def delete_email_account(account_id: str, owner: str = Depends(require_user)):
        ...
        try:
            ...
            if was_default:
                ...
                promote_q = _email_account_scope_for_owner(promote_q, EmailAccount, owner)
