...
_PDF_EXTRACT_HEAD_RE = re.compile(r'^(<!--[^>]+-->\s*\n+#[^\n]*\n+)', re.MULTILINE)


def _pdf_extract_head(content: str, title: str | None) -> str:
    content = content or ""
    head_match = _PDF_EXTRACT_HEAD_RE.match(content)
    if head_match:
...
def _parse_tidy_verdicts(response: str) -> List[Any]:
    text = response or ""
