...
//  document.getElementById('chat-form').addEventListener('submit', chatModule.handleChatSubmit);
  ...
  if (exportPdfBtn) {
    exportPdfBtn.addEventListener('click', (e) => {
      ...
      const sessionName = meta ? meta.name : 'CyberVault Chat';
  ...
  document.addEventListener('click', (e) => {
    ...
    if (!modal || modal.classList.contains('hidden')) return;
    if (modal.classList.contains('cybervault-route-view')) return;
    if (e.target.closest('.modal-content')) return;
    dismissModal(modal);
  });

  // CyberVault shell: when the backdrop is visually dimmed but pointer events
  // land on the underlying sidebar/page, still treat that as an outside click.
  document.addEventListener('click', (e) => {
    if (uiModule.isTouchInsideModal()) return;
    if (e.target.closest('.modal, .modal-content, .modal-dock, .minimized-dock, .minimized-dock-chip, .modal-dock-item')) return;
    const openModals = Array.from(document.querySelectorAll('.modal:not(.hidden):not(.modal-minimized):not(.minimized)'))
      .filter((m) => {
        if (m.id === 'styled-confirm-overlay') return false;
        if (m.classList.contains('cybervault-route-view')) return false;
        const cs = window.getComputedStyle(m);
        return cs.display !== 'none' && cs.visibility !== 'hidden';
      });
    if (!openModals.length) return;
    openModals.sort((a, b) => (parseInt(getComputedStyle(a).zIndex, 10) || 0) - (parseInt(getComputedStyle(b).zIndex, 10) || 0));
    const topModal = openModals[openModals.length - 1];
    const ignoreUntil = Number(topModal.dataset?.ignoreShellOutsideClickUntil || 0);
  ...
  const _collapseSidebarToRail = () => {
    ...
    if (!sb || !rail) return;
    // Route navigation used to collapse the full sidebar into the icon rail.
    // Keep the user's current sidebar state instead; only explicit hamburger,
    // backdrop, or swipe-close actions should hide it.
  ...
  const _routeToolModalIds = ['memory-modal', 'calendar-modal', 'cookbook-modal', 'gallery-modal', 'tasks-modal', 'library-modal', 'archive-modal', 'doclib-modal', 'email-lib-modal', 'theme-modal', 'research-overlay', 'compare-model-overlay'];
  function _clearRouteViewClasses() {
    _routeToolModalIds.forEach((id) => {
      const modal = document.getElementById(id);
      if (!modal) return;
      modal.classList.remove('cybervault-route-view');
      modal.querySelector('.modal-content, .cal-modal-content, .memory-modal-content, .doclib-modal-content, .gallery-modal-content, .tasks-modal-content')
        ?.classList.remove('cybervault-route-content');
    });
    document.getElementById('notes-pane-backdrop')?.classList.remove('cybervault-route-view', 'notes-route-view');
    document.getElementById('notes-pane')?.classList.remove('cybervault-route-view', 'notes-route-pane');
  }
  function _closeRouteToolViews(exceptId) {
    _clearRouteViewClasses();
    if (exceptId !== 'memory-modal') {
      const mem = document.getElementById('memory-modal');
      if (mem) mem.classList.add('hidden');
    }
    if (exceptId !== 'calendar-modal' && calendarModule?.isCalendarOpen?.()) calendarModule.closeCalendar();
    if (exceptId !== 'cookbook-modal' && cookbookModule?.isVisible?.()) cookbookModule.close();
    if (exceptId !== 'gallery-modal' && galleryModule?.isGalleryOpen?.()) galleryModule.closeGallery();
    if (exceptId !== 'tasks-modal' && tasksModule?.isTasksOpen?.()) tasksModule.closeTasks();
    if (exceptId !== 'library-modal') {
      try { sessionModule?.closeLibrary?.(); } catch (_) {}
    }
    if (exceptId !== 'doclib-modal') {
      const doclibModal = document.getElementById('doclib-modal');
      if (doclibModal) {
        doclibModal.classList.add('hidden');
        doclibModal.classList.remove('cybervault-route-view');
        doclibModal.querySelector('.modal-content, .doclib-modal-content')
          ?.classList.remove('cybervault-route-content');
        doclibModal.style.display = 'none';
      }
    }
    if (exceptId !== 'archive-modal') {
      try { sessionModule?.closeArchive?.(); } catch (_) {}
    }
    if (exceptId !== 'email-lib-modal') {
      const emailModal = document.getElementById('email-lib-modal');
      if (emailModal) {
        emailModal.classList.add('hidden');
        emailModal.classList.remove('email-lib-fullscreen', 'cybervault-route-view');
        emailModal.querySelector('.modal-content, .doclib-modal-content')
          ?.classList.remove('cybervault-route-content');
        emailModal.style.display = 'none';
      }
    }
    if (exceptId !== 'compare-mode' && compareModule?.isActive?.()) {
      try { compareModule.deactivate(true); } catch (_) {}
    }
    if (exceptId !== 'research-panel' && researchPanelModule?.isOpen?.()) {
      try { researchPanelModule.closePanel(); } catch (_) {}
    }
    if (exceptId !== 'research-panel') {
      const researchOverlay = document.getElementById('research-overlay');
      if (researchOverlay) researchOverlay.classList.remove('cybervault-route-view');
    }
    if (exceptId !== 'theme-modal') {
      const themeModal = document.getElementById('theme-modal');
      if (themeModal) themeModal.classList.add('hidden');
    }
    try {
      if (exceptId !== 'notes-pane' && notesModule?.isPanelOpen?.()) notesModule.closePanel();
    } catch (_) {}
  }
  function _promoteRouteModal(modalId) {
    const apply = () => {
      const modal = document.getElementById(modalId);
      if (!modal || modal.classList.contains('hidden')) return false;
      modal.classList.add('cybervault-route-view');
      modal.querySelector('.modal-content, .cal-modal-content, .memory-modal-content, .doclib-modal-content, .gallery-modal-content, .tasks-modal-content')
        ?.classList.add('cybervault-route-content');
      return true;
    };
    if (apply()) return;
  ...
  function _routeNotesFullscreen() {
    const go = () => {
      ...
      pane.classList.add('cybervault-route-view', 'notes-route-pane');
      if (backdrop) backdrop.classList.add('cybervault-route-view', 'notes-route-view');
      return true;
    };
    if (!go()) {
  ...
  function _routeEmailFullscreen() {
    const go = () => {
      if (window.location.pathname !== '/email') return false;
      const modal = document.getElementById('email-lib-modal');
      if (!modal) return false;
      modal.style.display = '';
      modal.classList.remove('hidden');
      modal.classList.add('email-lib-fullscreen');
      _promoteRouteModal('email-lib-modal');
      return true;
    };
    go();
    requestAnimationFrame(go);
  ...
  function _openEmailLibraryForRoute() {
    const fallbackHeader = () => {
      const hdr = document.querySelector('#email-section .section-header-flex');
      if (hdr) hdr.click();
      _routeEmailFullscreen();
    };
    import('./js/emailLibrary.js')
      .then((mod) => {
        const open = mod.openEmailLibrary || (mod.default && mod.default.openEmailLibrary) || window.openEmailLibrary;
      ...
      .catch(() => fallbackHeader());
  }
  function _focusDashboardSearch() {
    _ensureCyberVaultDashboard();
    const input = el('dashboard-page-search-input') || el('dashboard-search-input');
    if (!input) return;
  ...
  function _dashboardCountFromPayload(data, key) {
    if (!data) return 0;
    if (Array.isArray(data)) return data.length;
    if (Array.isArray(data.items)) {
      if (key === 'models') {
        return data.items.reduce((sum, ep) => sum + (Array.isArray(ep?.models) ? ep.models.length : 0), 0);
      }
    ...
    return Number(data.count || data.total || 0) || 0;
  }
  function _setDashboardValue(key, value) {
    const node = document.querySelector(`[data-dashboard-value="${key}"]`);
    if (!node) return;
    node.textContent = String(Math.max(0, Number(value) || 0));
  }
  function _dashboardTimestamp(item) {
    if (!item || typeof item !== 'object') return null;
    return item.updated_at || item.created_at || item.timestamp || item.last_message_at || item.modified_at || null;
  }
  function _bucketDashboardItems(items) {
    const buckets = Array(8).fill(0);
    const now = Date.now();
    const day = 24 * 60 * 60 * 1000;
    (items || []).forEach((item) => {
      const ts = Date.parse(_dashboardTimestamp(item) || '');
      if (!Number.isFinite(ts)) return;
      const age = Math.max(0, Math.min(7, Math.floor((now - ts) / day)));
      buckets[7 - age] += 1;
    });
    return buckets.some(Boolean) ? buckets : null;
  }
  function _dashboardPayloadSeries(data, key, value) {
    if (!data) return null;
    if (key === 'models' && Array.isArray(data.items)) {
      const counts = data.items.map((endpoint) => Array.isArray(endpoint?.models) ? endpoint.models.length : 0);
      return counts.length ? counts.slice(-8) : null;
    }
    if (key === 'tools') {
      const tools = Array.isArray(data.tools) ? data.tools : Array.isArray(data.items) ? data.items : Array.isArray(data) ? data : null;
      if (tools && tools.length) {
        const grouped = new Map();
        tools.forEach((tool) => {
          const group = tool?.group || tool?.category || tool?.type || tool?.namespace || 'tools';
    ...
    if (key === 'sessions') {
      const sessions = Array.isArray(data.sessions) ? data.sessions : Array.isArray(data.items) ? data.items : Array.isArray(data) ? data : null;
      return _bucketDashboardItems(sessions);
    }
    if (key === 'documents') {
      const docs = Array.isArray(data.documents) ? data.documents : Array.isArray(data.items) ? data.items : Array.isArray(data) ? data : null;
      return _bucketDashboardItems(docs);
    }
    return Number.isFinite(Number(value)) ? [Number(value)] : null;
  }
  function _dashboardCountHistory(key, value) {
    const n = Math.max(0, Number(value) || 0);
    const storeKey = `cybervault-dashboard-history-${key}`;
    let series = [];
    try {
      series = JSON.parse(sessionStorage.getItem(storeKey) || '[]');
      if (!Array.isArray(series)) series = [];
    } catch (_) {
      series = [];
    }
    if (!series.length || series[series.length - 1] !== n) series.push(n);
    series = series.slice(-8);
    try { sessionStorage.setItem(storeKey, JSON.stringify(series)); } catch (_) {}
    return series;
  }
  function _sparkPathFromSeries(series) {
    const values = (series || []).map((v) => Math.max(0, Number(v) || 0));
    while (values.length < 8) values.unshift(values.length ? values[0] : 0);
    const min = Math.min(...values);
    const max = Math.max(...values);
    const spread = Math.max(1, max - min);
    return values.map((v, i) => {
      const x = 2 + (i * (116 / Math.max(1, values.length - 1)));
      const y = 29 - (((v - min) / spread) * 22);
      return `${i ? 'L' : 'M'}${x.toFixed(1)} ${y.toFixed(1)}`;
    }).join(' ');
  }
  function _updateDashboardSpark(key, value, payload) {
    const card = document.querySelector(`[data-dashboard-stat="${key}"]`);
    const path = card?.querySelector('.cybervault-dashboard-spark path');
    if (!path) return;
    const payloadSeries = _dashboardPayloadSeries(payload, key, value);
    const historySeries = _dashboardCountHistory(key, value);
    const series = payloadSeries && payloadSeries.some((v) => Number(v) > 0) ? payloadSeries : historySeries;
    path.setAttribute('d', _sparkPathFromSeries(series));
    path.dataset.points = (series || []).join(',');
    path.closest('svg')?.classList.toggle('is-flat', new Set(series || []).size <= 1);
  }
  function _setDashboardStat(key, value, payload) {
    _setDashboardValue(key, value);
    _updateDashboardSpark(key, value, payload);
  }
  function _bindDashboardInlineSearch(input) {
    if (!input || input.dataset.cybervaultSearchBound === '1') return;
    if (searchChatModule?.bindInlineSearch) {
      const host = input.closest('.cybervault-dashboard-search') || input.parentElement;
      let panel = el('dashboard-page-search-results');
      if (!panel && host) {
        panel = document.createElement('div');
        panel.id = 'dashboard-page-search-results';
        panel.className = 'dashboard-search-results cybervault-dashboard-search-results search-results hidden';
        panel.setAttribute('role', 'listbox');
        host.insertAdjacentElement('afterend', panel);
      }
      if (input.dataset.inlineSearchBound === '1' && input.getAttribute('aria-controls') !== 'dashboard-page-search-results') {
        delete input.dataset.inlineSearchBound;
      }
      searchChatModule.bindInlineSearch(input, { host, panel });
      input.dataset.cybervaultSearchBound = '1';
    }
  }
  function _refreshCyberVaultDashboardStats() {
    _setDashboardStat('tools', document.querySelectorAll('#tools-section .tool-item, #icon-rail .icon-rail-btn').length || 10);
    fetch(`${API_BASE}/api/models`, { credentials: 'same-origin' })
      .then((r) => r.ok ? r.json() : null)
      .then((data) => _setDashboardStat('models', _dashboardCountFromPayload(data, 'models'), data))
      .catch(() => {});
    fetch(`${API_BASE}/api/tools`, { credentials: 'same-origin' })
      .then((r) => r.ok ? r.json() : null)
      .then((data) => {
        const count = _dashboardCountFromPayload(data, 'tools');
        if (count) _setDashboardStat('tools', count, data);
      })
      .catch(() => {});
    fetch(`${API_BASE}/api/sessions`, { credentials: 'same-origin' })
      .then((r) => r.ok ? r.json() : null)
      .then((data) => _setDashboardStat('sessions', _dashboardCountFromPayload(data, 'sessions'), data))
      .catch(() => {});
    fetch(`${API_BASE}/api/documents/library?limit=1`, { credentials: 'same-origin' })
      .then((r) => r.ok ? r.json() : null)
      .then((data) => _setDashboardStat('documents', _dashboardCountFromPayload(data, 'documents'), data))
      .catch(() => {});
  }
  function _ensureCyberVaultDashboard() {
    const page = el('cybervault-dashboard-page');
    if (!page) return null;
    page.classList.remove('hidden');
    _bindDashboardInlineSearch(el('dashboard-page-search-input'));
    if (!page.dataset.cybervaultStatsBound) {
      page.dataset.cybervaultStatsBound = '1';
      _refreshCyberVaultDashboardStats();
      setInterval(_refreshCyberVaultDashboardStats, 30000);
    }
    return page;
  }
  const _routeChromeTargets = new Map([
    ['/dashboard', ['sidebar-search-btn']],
    ...
    ['/documents', ['tool-library-btn', 'rail-documents']],
    ['/library', ['rail-archive']],
    ['/notes', ['tool-notes-btn', 'rail-notes']],
    ['/theme', ['tool-theme-btn', 'rail-theme']],
    ['/email', ['email-section-title', 'rail-email']],
  ]);
  function _syncRouteChrome(path) {
    const route = path || '/';
    document.body.classList.toggle('cybervault-dashboard-route', route === '/dashboard');
    document.body.classList.toggle('cybervault-chat-route', route === '/');
    document.body.dataset.cybervaultRoute = route;
    const allIds = new Set();
    _routeChromeTargets.forEach((ids) => ids.forEach((id) => allIds.add(id)));
    allIds.forEach((id) => {
      const node = el(id);
      if (!node) return;
      node.classList.remove('active');
      node.setAttribute('aria-current', 'false');
      const row = node.closest?.('.section-header-flex');
      row?.classList.remove('active');
      row?.setAttribute('aria-current', 'false');
    });
    const activeIds = _routeChromeTargets.get(route) || [];
  ...
  function _openRoutePath(path) {
    const _sidebarWasOpenForRoute = !document.getElementById('sidebar')?.classList.contains('hidden');
    const _preserveRouteSidebar = () => {
      if (!_sidebarWasOpenForRoute) return;
      const sb = document.getElementById('sidebar');
      const rail = document.getElementById('icon-rail');
      if (!sb) return;
      sb.classList.remove('hidden');
      if (rail) rail.classList.remove('rail-hidden');
      delete document.body.dataset.routeCollapsedSidebar;
      try { window.syncRailSide && window.syncRailSide(); } catch (_) {}
    };
    const openers = {
      '/notes': () => {
        _closeRouteToolViews('notes-pane');
        _collapseSidebarToRail();
        notesModule?.openPanel?.();
        _routeNotesFullscreen();
      },
      '/calendar': () => {
        _collapseSidebarToRail();
        _closeRouteToolViews('calendar-modal');
        calendarModule?.openCalendar?.();
        _promoteRouteModal('calendar-modal');
      },
      '/compare': () => {
        _collapseSidebarToRail();
        _closeRouteToolViews('compare-mode');
        if (compareModule && !compareModule.isActive?.()) {
          try {
            window._cybervaultSuppressRouteReturn = true;
            _startFreshChat();
            Promise.resolve(compareModule.toggleMode?.()).finally(() => {
              _promoteRouteModal('compare-model-overlay');
              try { delete window._cybervaultSuppressRouteReturn; } catch (_) {}
            });
          } catch (_) {
            try { delete window._cybervaultSuppressRouteReturn; } catch (_) {}
          }
      ...
      '/email': () => {
        ...
        _collapseSidebarToRail();
        _openEmailLibraryForRoute();
      },
      '/research': () => {
        _collapseSidebarToRail();
        _closeRouteToolViews('research-panel');
        if (!researchPanelModule?.isOpen?.()) researchPanelModule?.openPanel?.();
        _promoteRouteModal('research-overlay');
      },
      '/memory': () => {
        _collapseSidebarToRail();
        _closeRouteToolViews('memory-modal');
        if (memoryModal) {
          memoryModal.classList.remove('hidden');
          memoryModule?.renderMemoryList?.();
          memoryModule?.updateMemoryCount?.();
          _promoteRouteModal('memory-modal');
        }
      },
      '/gallery': () => {
        _collapseSidebarToRail();
        _closeRouteToolViews('gallery-modal');
        galleryModule?.openGallery?.();
        _promoteRouteModal('gallery-modal');
      },
      '/tasks': () => {
        _collapseSidebarToRail();
        _closeRouteToolViews('tasks-modal');
        tasksModule?.openTasks?.();
        _promoteRouteModal('tasks-modal');
      },
      '/theme': () => {
        _collapseSidebarToRail();
        _closeRouteToolViews('theme-modal');
        const themeModal = document.getElementById('theme-modal');
        if (themeModal) {
          themeModal.classList.remove('hidden');
          _promoteRouteModal('theme-modal');
        }
      },
      '/library': () => {
        _collapseSidebarToRail();
        _closeRouteToolViews('library-modal');
        sessionModule?.openLibrary?.();
        _promoteRouteModal('library-modal');
        _promoteRouteModal('doclib-modal');
      },
      '/documents': () => {
        _collapseSidebarToRail();
        _closeRouteToolViews('doclib-modal');
        sessionModule?.openLibrary?.('documents');
        _promoteRouteModal('doclib-modal');
      },
      '/dashboard': () => {
        _closeRouteToolViews();
        _restoreSidebarIfRouteCollapsed();
        _focusDashboardSearch();
      },
      '/': () => {
        _closeRouteToolViews();
        _restoreSidebarIfRouteCollapsed();
    ...
    (openers[path] || openers['/'])();
    _syncRouteChrome(path);
    if (path !== '/') {
      requestAnimationFrame(_preserveRouteSidebar);
      setTimeout(_preserveRouteSidebar, 50);
      setTimeout(_preserveRouteSidebar, 250);
    }
  }
  const _routeOpen = {
    '/notes':    () => {
    ...
    '/email':    () => {
      ...
      try {
        ...
        setTimeout(() => { try { delete window._cybervaultSuppressRouteReturn; } catch (_) {} }, 0);
      }
  ...
  Object.assign(_routeOpen, {
    '/notes':    () => _openRoutePath('/notes'),
    '/calendar': () => _openRoutePath('/calendar'),
    '/compare':  () => _openRoutePath('/compare'),
    '/cookbook': () => _openRoutePath('/cookbook'),
    '/email':    () => _openRoutePath('/email'),
    '/research': () => _openRoutePath('/research'),
    '/memory':   () => _openRoutePath('/memory'),
    '/gallery':  () => _openRoutePath('/gallery'),
    '/tasks':    () => _openRoutePath('/tasks'),
    '/theme':    () => _openRoutePath('/theme'),
    '/library':  () => _openRoutePath('/library'),
    '/documents': () => _openRoutePath('/documents'),
    '/dashboard': () => _openRoutePath('/dashboard'),
  });
  const _routeClickMap = new Map([
    ['sidebar-search-btn', '/dashboard'],
  ...
  const _routeClickSelector = Array.from(_routeClickMap.keys()).map((id) => `#${id}`).join(',');
  function _navigateRoute(path) {
    if (window.location.pathname !== path) {
      window.history.pushState({ cybervaultRoute: path }, '', path);
    }
    _openRoutePath(path);
  }
  function _returnToChatRoute() {
    if (window.location.pathname !== '/') {
      window.history.pushState({ cybervaultRoute: '/' }, '', '/');
    }
    _openRoutePath('/');
  }
  function _maybeReturnToChatRoute() {
    if (!window._cybervaultSuppressRouteReturn) _returnToChatRoute();
  }
  window._cybervaultReturnToChatRoute = _returnToChatRoute;
  function _expandSidebarFromRail() {
    const sb = document.getElementById('sidebar');
    const rail = document.getElementById('icon-rail');
    if (!sb || !sb.classList.contains('hidden')) return;
    sb.classList.remove('hidden');
    document.body.classList.remove('sidebar-collapsed', 'hamburger-only');
    delete document.body.dataset.routeCollapsedSidebar;
    if (rail) rail.classList.add('rail-hidden');
    try { window.syncRailSide && window.syncRailSide(); } catch (_) {}
  }
  document.addEventListener('click', (e) => {
    if (e.target.closest?.('.icon-rail-btn')) _expandSidebarFromRail();
  }, true);
  document.addEventListener('click', (e) => {
    const routeClose = e.target.closest?.(
      '#doclib-modal.cybervault-route-view #doclib-close,' +
      '#doclib-modal.cybervault-route-view .close-btn,' +
      '#doclib-modal.cybervault-route-view .modal-close'
    );
    if (!routeClose) return;
    e.preventDefault();
    e.stopImmediatePropagation();
    try { documentModule?.closeLibrary?.({ returnToChatRoute: true }); } catch (_) {}
    try { sessionModule?.closeLibrary?.(); } catch (_) {}
  ...
  document.addEventListener('click', (e) => {
    ...
    const emailRouteRow = e.target.closest?.('#email-section .section-header-flex');
    if (emailRouteRow && !e.target.closest?.('#email-compose-btn')) {
      e.preventDefault();
      e.stopImmediatePropagation();
      _navigateRoute('/email');
      return;
    }
    const btn = e.target.closest(_routeClickSelector);
    if (!btn) return;
    const path = _routeClickMap.get(btn.id);
    if (!path) return;
    e.preventDefault();
    e.stopImmediatePropagation();
    if (btn.id && btn.id.startsWith('rail-')) _expandSidebarFromRail();
    _navigateRoute(path);
  }, true);
  window.addEventListener('popstate', () => _openRoutePath(window.location.pathname || '/'));
  window._cybervaultOpenRoutePath = _navigateRoute;
  _syncRouteChrome(urlPath || '/');
...
// Voice recording is handled by the dual-purpose send/mic button (see below)
  ...
  (function initModelPickerResponsive() {
    ...
    function checkPickerOverflow() {
      ...
      if (textarea) {
        textarea.setAttribute('placeholder', w < PLACEHOLDER_HIDE_WIDTH ? '' : 'Message CyberVault...');
      }
  ...
  if (incognitoBtn) {
    ...
    incognitoBtn.addEventListener('click', () => {
      ...
      const tipEl = el('welcome-tip');
      incognitoBtn.title = chk.checked ? 'Disable Anonymous mode' : 'Enable Anonymous mode — no memory, no history saved';
      const welcomeName = document.querySelector('.welcome-name');
      if (chk.checked) {
        incognitoBtn.innerHTML = INCOGNITO_EYE_CLOSED + '<span class="incognito-label"></span>';
        if (welcomeName) {
          welcomeName.dataset.originalHtml = welcomeName.innerHTML;
          welcomeName.innerHTML = '<svg class="welcome-boat" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><line x1="8" y1="16" x2="16" y2="8"/><line x1="8" y1="8" x2="16" y2="16"/></svg>Anonymous';
        ...
        if (welcomeSub) {
          ...
          welcomeSub.textContent = "Who am I? I'm Anonymous.";
        ...
        incognitoBtn.innerHTML = INCOGNITO_EYE_OPEN + '<span class="incognito-label"></span>';
...
function startOdysseusApp() {
  ...
  if (sidebarSearchBtn) {
    const sidebarNewChatBtnForOrder = el('sidebar-new-chat-btn');
    if (sidebarNewChatBtnForOrder && sidebarSearchBtn.parentElement === sidebarNewChatBtnForOrder.parentElement) {
      sidebarNewChatBtnForOrder.parentElement.insertBefore(sidebarSearchBtn, sidebarNewChatBtnForOrder);
    }
    sidebarSearchBtn.title = 'Dashboard';
    sidebarSearchBtn.setAttribute('aria-label', 'Dashboard');
    sidebarSearchBtn.dataset.cybervaultRoute = '/dashboard';
    const dashboardHomeIcon = '<svg class="sidebar-action-icon" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="m3 10.5 9-7 9 7"/><path d="M5 9.5V20h14V9.5"/><path d="M9 20v-6h6v6"/></svg>';
    const currentDashboardIcon = sidebarSearchBtn.querySelector('svg');
    if (currentDashboardIcon) currentDashboardIcon.outerHTML = dashboardHomeIcon;
    else sidebarSearchBtn.insertAdjacentHTML('afterbegin', dashboardHomeIcon);
    sidebarSearchBtn.querySelectorAll('span').forEach((span) => {
      if ((span.textContent || '').trim().toLowerCase() === 'search') span.textContent = 'Dashboard';
    });
  }
  const dashboardSearchInput = el('dashboard-search-input');
  if (dashboardSearchInput) {
    if (searchChatModule && searchChatModule.bindInlineSearch) {
      searchChatModule.bindInlineSearch(dashboardSearchInput);
    }
  }
  const dashboardPageSearchInput = el('dashboard-page-search-input');
  if (typeof _bindDashboardInlineSearch === 'function') {
    _bindDashboardInlineSearch(dashboardPageSearchInput);
  } else if (dashboardPageSearchInput && searchChatModule?.bindInlineSearch) {
    const host = dashboardPageSearchInput.closest('.cybervault-dashboard-search') || dashboardPageSearchInput.parentElement;
    let panel = el('dashboard-page-search-results');
    if (!panel && host) {
      panel = document.createElement('div');
      panel.id = 'dashboard-page-search-results';
    ...
    if (dashboardPageSearchInput.dataset.inlineSearchBound === '1' && dashboardPageSearchInput.getAttribute('aria-controls') !== 'dashboard-page-search-results') {
      delete dashboardPageSearchInput.dataset.inlineSearchBound;
    }
    searchChatModule.bindInlineSearch(dashboardPageSearchInput, { host, panel });
    dashboardPageSearchInput.dataset.cybervaultSearchBound = '1';
  }
  ...
  function _dockModelPickerInComposerControls() {
    const picker = document.getElementById('model-picker-wrap');
    const right = document.querySelector('.chat-input-right');
    const modeToggle = document.getElementById('agent-mode-toggle');
    if (!picker || !right) return;
    if (picker.parentElement !== right) {
      right.insertBefore(picker, modeToggle || right.firstChild);
      picker.classList.add('composer-model-picker');
    }
  }
  _dockModelPickerInComposerControls();
  requestAnimationFrame(_dockModelPickerInComposerControls);
  try {
    new MutationObserver(_dockModelPickerInComposerControls).observe(document.body, { childList: true, subtree: true });
  } catch (_) {}
