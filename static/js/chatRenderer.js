...
const EXECUTABLE_TOOL_FENCE_RE = /```(?:bash|python)\s*\n[\s\S]*?```/gi;

function stripToolResultNarration(text) {
  const lines = String(text || '').split('\n');
  const kept = [];
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i];
    const toolSection = line.match(/^\s*(?:[-*]\s*)?(?:\*\*)?(stdout|stderr)(?:\*\*)?\s*:\s*(.*)$/i);
    if (toolSection) {
      const rest = toolSection[2] || '';
      const fenceStart = rest.includes('```') || (lines[i + 1] || '').trim().startsWith('```');
      if (fenceStart) {
        if (!rest.includes('```')) i += 1;
    ...
    if (/^\s*(?:[-*]\s*)?(?:\*\*)?exit_code(?:\*\*)?\s*:\s*(?:`?[-\w]+`?)\s*$/i.test(line)) {
      continue;
    }
    kept.push(line);
  }
...
function _billableCost(model, inputTokens, outputTokens, providerCost) {
  const url = _currentEndpointUrl();
  if (!isCostTrackedEndpoint(url)) return null;
  const upstreamCost = _providerCost(providerCost);
  if (upstreamCost !== null) return upstreamCost;
...
function composerContextSlot() {
  ...
  if (!left) return null;
  let slot = document.getElementById('composer-context-meter');
  const metricsSlot = document.getElementById('composer-response-metrics');
  if (!slot) {
    slot = document.createElement('span');
    slot.id = 'composer-context-meter';
    slot.className = 'composer-context-meter';
  }
  if (metricsSlot && metricsSlot.parentElement === left) {
    const next = metricsSlot.nextSibling;
    if (next !== slot) left.insertBefore(slot, next);
...
function composerMetricsSlot() {
  const left = document.querySelector('.chat-input-left');
  if (!left) return null;
  let slot = document.getElementById('composer-response-metrics');
  if (!slot) {
    slot = document.createElement('span');
    slot.id = 'composer-response-metrics';
    slot.className = 'composer-response-metrics';
    left.insertBefore(slot, document.getElementById('web-toggle-btn') || left.firstChild);
  }
  return slot;
}

function dockResponseMetrics(metricsEl) {
  if (!metricsEl) return false;
  const slot = composerMetricsSlot();
  if (!slot) return false;
  slot.querySelectorAll('.response-metrics').forEach((node) => node.remove());
  slot.appendChild(metricsEl);
  slot.classList.remove('hidden');
  composerContextSlot();
  return true;
}

function dockContextRing(ctxRing) {
  if (!ctxRing) return false;
  const slot = composerContextSlot();
  if (!slot) return false;
  slot.querySelectorAll('.ctx-ring').forEach((node) => node.remove());
  slot.appendChild(ctxRing);
  slot.classList.remove('hidden');
  return true;
}
...
export function stripToolBlocks(text) {
  ...
  cleaned = stripToolResultNarration(cleaned);
  cleaned = cleaned.replace(TOOL_NARRATION_RE, '');
  cleaned = cleaned.replace(/\n{3,}/g, '\n\n');
  return cleaned.trim();
}

export function stripAgentToolChatter(text) {
  let cleaned = stripToolBlocks(text || '');
  cleaned = cleaned.replace(EXECUTABLE_TOOL_FENCE_RE, '');
  cleaned = stripToolResultNarration(cleaned);
  cleaned = cleaned.replace(TOOL_NARRATION_RE, '');
  cleaned = cleaned.replace(/\n{3,}/g, '\n\n');
  return cleaned.trim();
...
export function showWelcomeScreen() {
  ...
  if (cc) {
    cc.classList.add('welcome-active');
    cc.classList.remove('chat-flow-active');
  }
  document.body?.classList.remove('chat-flow-active');
...
let _chatFlowObserver = null;

function hasVisibleChatOutput(box) {
  if (!box) return false;
  return !!box.querySelector('.msg, .agent-thread, .ask-user-card, .generated-image-wrap');
}

export function syncChatFlowState() {
  const box = document.getElementById('chat-history');
  const cc = document.getElementById('chat-container');
  const ws = document.getElementById('welcome-screen');
  const hasOutput = hasVisibleChatOutput(box);

  if (cc) {
    cc.classList.toggle('chat-flow-active', hasOutput);
    cc.classList.toggle('welcome-active', !hasOutput && !ws?.classList.contains('hidden'));
  }
  document.body?.classList.toggle('chat-flow-active', hasOutput);
  if (ws) ws.classList.toggle('hidden', hasOutput);

  if (!_chatFlowObserver && box && typeof MutationObserver !== 'undefined') {
    _chatFlowObserver = new MutationObserver(() => syncChatFlowState());
    _chatFlowObserver.observe(box, { childList: true, subtree: false });
  }
}
...
export function displayMetrics(messageElement, metrics) {
  ...
  let ctxPct = metrics.context_percent;
  if ((ctxPct === undefined || ctxPct === null) && metrics.context_length && inputTokens) {
    ctxPct = Math.max(0.1, Math.min(100, (inputTokens / metrics.context_length) * 100));
  }
  const model = metrics.model || 'Unknown';
  const cost = _billableCost(model, inputTokens, outputTokens, metrics.provider_cost);
  ...
  if (footer) {
    ...
    if (ctxRing && !dockContextRing(ctxRing)) messageElement.appendChild(ctxRing);
  }
  ...
  try {
    ...
    if (role === 'assistant' && metadata && metadata.tool_events && metadata.tool_events.length > 0) {
      ...
      for (let r = 0; r < maxRound; r++) {
        const roundNum = r + 1;
        const txt = stripAgentToolChatter(roundTexts[r] || '').trim();
    ...
    const wrap = document.createElement('div');
    const isGroupReply = role === 'assistant' && !!metadata?.group_model;
    wrap.className = 'msg ' + (role === 'user' ? 'msg-user' : 'msg-ai') + (isGroupReply ? ' msg-group' : '');
