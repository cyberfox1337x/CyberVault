...
function _dismissCookbookTaskDropdowns(root = document) {
  root.querySelectorAll?.('.cookbook-task-dropdown').forEach(dismissOrRemove);
}
...
export function _renderRunningTab() {
  ...
  if (!hasContent) {
    _dismissCookbookTaskDropdowns(group);
    group.remove();
    return;
  }
  ...
  group.querySelectorAll('.cookbook-serve-section, .cookbook-dl-section').forEach(el => {
    _dismissCookbookTaskDropdowns(el);
    el.remove();
  });
  ...
  (_adminCard || group).querySelectorAll('[class*="cookbook-server-section-"]').forEach(el => {
    ...
    if (cls && !_liveSafeKeys.has(cls.replace('cookbook-server-section-', ''))) {
      _dismissCookbookTaskDropdowns(el);
      el.remove();
    }
  });
  ...
  group.querySelectorAll('[data-clear-server]').forEach(btn => {
    ...
    btn.addEventListener('click', async (e) => {
      ...
      setTimeout(() => {
        toRemove.forEach(t => {
          const card = document.querySelector(`.cookbook-task[data-task-id="${t.sessionId}"]`);
          if (card) {
            _dismissCookbookTaskDropdowns(card);
            card.remove();
          }
        });
        ...
        if (_sec && !_sec.querySelector('.cookbook-task')) {
          _dismissCookbookTaskDropdowns(_sec);
          _sec.remove();
        }
  ...
  for (const task of tasks) {
    ...
    if (menuBtn) {
      ...
      menuBtn.addEventListener('click', (e) => {
        ...
        dropdown.dataset.taskId = task.sessionId || '';
        ...
        let _bindTimer = null;
        const _cleanup = () => {
          if (_bindTimer) { clearTimeout(_bindTimer); _bindTimer = null; }
          _unreg(); _unreg = () => {};
          dropdown.remove();
