// Model Picker â€” chatbox model selector dropdown
...
// â”€â”€ Recent + Favorites persistence â”€â”€
...
// behind search would be a regression â€” keep listing them in browse mode.
const BROWSE_ALL_LIMIT = 12;

const PROVIDER_CLASS_ALIAS = {
  'meta-llama': 'meta',
...
function _modelProviderSlug(mid) {
  if (!mid || typeof mid !== 'string') return 'other';
  if (/^(gpt|chatgpt|o[0-9]|openai)([-_:.]|$)/i.test(mid)) return 'openai';
  if (/^claude([-_:.]|$)/i.test(mid)) return 'anthropic';
  if (/^(gemini|palm)([-_:.]|$)/i.test(mid)) return 'google';
  if (/^deepseek([-_:.]|$)/i.test(mid)) return 'deepseek-ai';
  if (/^mistral([-_:.]|$)/i.test(mid)) return 'mistralai';
  const slash = mid.indexOf('/');
  const slug = slash > 0 ? mid.substring(0, slash) : 'other';
  return PROVIDER_CLASS_ALIAS[slug] || slug;
}

function _modelProviderClass(mid) {
  return 'provider-' + _modelProviderSlug(mid).replace(/[^a-z0-9_-]/gi, '-').toLowerCase();
}

function _chatGptTierInfo(mid) {
  const id = String(mid || '').toLowerCase();
  if (id.includes('gpt-5.3-codex')) return { emoji: '💻', label: 'Coding', display: 'gpt-5.3-codex', cls: 'codex' };
  if (id === 'gpt-5.4-mini' || id.includes('gpt-5.4-mini')) return { emoji: '⚡', label: 'Mini / Fast', display: 'gpt-5.4-mini', cls: 'mini' };
  if (id === 'gpt-5.4' || id.endsWith('/gpt-5.4')) return { emoji: '🧠', label: 'Standard', display: 'gpt-5.4', cls: 'standard' };
  if (id === 'gpt-5.5' || id.endsWith('/gpt-5.5')) return { emoji: '👑', label: 'Advanced', display: 'gpt-5.5', cls: 'advanced' };
  return null;
}
...
// â”€â”€ Shared keyboard nav for model pickers â”€â”€
...
let _defaultChatPickPromise = null;
...
async function _ensureDefaultPendingChat(options = {}) {
  const { allowFallback = true } = options || {};
  if (!_deps) return false;
  if (_deps.getCurrentSessionId && _deps.getCurrentSessionId()) return true;
  const pending = _deps.getPendingChat && _deps.getPendingChat();
  if (pending && pending.modelId) return true;
  if (_defaultChatPickPromise) return _defaultChatPickPromise;
  _defaultChatPickInFlight = true;
  _defaultChatPickPromise = (async () => {
    let dc = null;
    try {
    ...
    if (allowFallback && window.modelsModule && window.modelsModule.getCachedItems) {
      const items = window.modelsModule.getCachedItems();
  ...
  try {
    return await _defaultChatPickPromise;
  } finally {
    _defaultChatPickInFlight = false;
    _defaultChatPickPromise = null;
  }
}
...
function _initModelPickerDropdown() {
  ...
  function _positionMenu() {
    ...
    const width = Math.min(360, Math.max(300, Math.min(window.innerWidth - 24, 360)));
    const availableAbove = Math.max(160, r.top - 22);
    const maxHeight = Math.min(360, availableAbove);
    menu.style.setProperty('position', 'fixed', 'important');
    menu.style.setProperty('width', `${width}px`, 'important');
    menu.style.setProperty('min-width', `${Math.min(width, 340)}px`, 'important');
    ...
    const top = Math.max(10, r.top - height - 10);
    menu.style.setProperty('left', `${left}px`, 'important');
    menu.style.setProperty('top', `${top}px`, 'important');
  }

  function _positionMenuAfterRender() {
    requestAnimationFrame(() => requestAnimationFrame(_positionMenu));
  }

  function _portalMenu() {
    if (menu.parentElement !== document.body) document.body.appendChild(menu);
    menu.classList.add('model-picker-portaled');
    _positionMenu();
    if (window.visualViewport && !_vvReposition) {
      _vvReposition = () => _positionMenu();
      window.visualViewport.addEventListener('resize', _vvReposition);
      window.visualViewport.addEventListener('scroll', _vvReposition);
    }
    window.addEventListener('resize', _positionMenu);
  }
  ...
  function _restoreMenu() {
    ...
    window.removeEventListener('resize', _positionMenu);
    menu.classList.remove('model-picker-portaled');
    ['position', 'width', 'min-width', 'max-width', 'max-height', 'overflow-y', 'right', 'bottom', 'left', 'top'].forEach((prop) => {
      menu.style.removeProperty(prop);
    });
    if (ownerWrap && menu.parentElement !== ownerWrap) ownerWrap.appendChild(menu);
  }
  ...
  // Local endpoint health â€” only probed for LOCAL endpoints, since
  ...
  function _getAllModels() {
    ...
    items.forEach(item => {
      ...
      // that briefly went down disappeared from the picker â€” confusing
      ...
      allModels.forEach((mid, i) => {
        // Deduplicate by model ID â€” prefer ONLINE endpoint entries over
  ...
  // â”€â”€ Provider display names and grouping â”€â”€
  ...
  function _providerClass(mid) {
    return 'provider-' + _providerSlug(mid).replace(/[^a-z0-9_-]/gi, '-').toLowerCase();
  }
  ...
  function _populate(filter) {
    ...
    function _addRow(m) {
      ...
      row.dataset.provider = _modelProviderSlug(m.mid);
      const tierInfo = _chatGptTierInfo(m.mid);
      if (tierInfo) row.classList.add('model-switch-chatgpt-tier', 'model-switch-tier-' + tierInfo.cls);
      if (m.stale) {
      ...
      const _mlogo = tierInfo ? tierInfo.emoji : providerLogo(m.mid);
      if (_mlogo) {
        const logoSpan = document.createElement('span');
        logoSpan.className = 'provider-logo ' + _modelProviderClass(m.mid) + (tierInfo ? ' model-tier-logo model-tier-' + tierInfo.cls + '-logo' : '');
        logoSpan.style.opacity = '0.6';
        if (tierInfo) logoSpan.textContent = _mlogo;
        else logoSpan.innerHTML = _mlogo;
        row.appendChild(logoSpan);
      }
      const nameSpan = document.createElement('span');
      nameSpan.className = 'mp-model-name';
      const modelDisplay = tierInfo ? tierInfo.display : m.display;
      nameSpan.textContent = modelDisplay;
      // Long model names are clipped with ellipsis â€” expose the full name on
      // hover so the suffix/variant tag is still discoverable (#1982).
      nameSpan.title = m.display;
      row.appendChild(nameSpan);
      // Offline state is already conveyed by the row's reduced opacity â€”
      ...
      epSpan.textContent = tierInfo ? tierInfo.label : _epDisplay;
      row.appendChild(epSpan);

      // Inline favorite dot â€” toggles favorite, never picks the model.
      const favDot = document.createElement('button');
      favDot.type = 'button';
      favDot.className = 'mp-fav-dot' + (favs.includes(m.mid) ? ' active' : '');
      favDot.textContent = 'â—';
      ...
      favDot.addEventListener('click', (e) => {
        ...
        // In browse mode the Favorites section membership changed â€” rebuild
    ...
    // â”€â”€ Search mode: flat, filtered results across the whole catalog â”€â”€
    if (q) {
    ...
    // â”€â”€ Browse mode: Favorites (manual) + Recent (auto), with dedupe. â”€â”€
    // Rules:
    //   1. Never list the same model twice in the dropdown. Favorites
    //      win over Recent (if you favorited it, that's where it
    //      belongs â€” Recent shouldn't show it again as duplicate).
    //   2. Small catalogs (â‰¤ BROWSE_ALL_LIMIT total) skip the Recent
    //      section entirely â€” when there's only ~10 models, the whole
  ...
  async function _pick(m) {
    ...
    // opens â€” the whole point of quick-switch.
    ...
    _close();
    // Refocus main textarea â€” skip on mobile to avoid keyboard bounce
    if (window.innerWidth >= 768) {
      const _ta = document.getElementById('message');
      if (_ta) setTimeout(() => _ta.focus(), 50);
    }
    if (!currentSessionId && _pendingChat) {
      // Already have a deferred session â€” just update the model
      _deps.setPendingChat({ url: m.url, modelId: m.mid, endpointId: m.endpointId });
      // Header stays as session name â€” model switch only updates picker
      updateModelPicker();
      uiModule.showToast(`Using ${m.display}`);
      return;
    } else if (!currentSessionId) {
      // No session yet â€” create one with this model
      await _deps.createDirectChat(m.url, m.mid, m.endpointId);
    } else {
      // Existing session with no model â€” PATCH it
      ...
      try {
        ...
        // Header stays as session name â€” model info shown in picker only
      } catch (e) {
        uiModule.showError('Failed to set model: ' + e);
        return;
      }
    }
    // Update picker visibility â€” model is now set
    updateModelPicker();
    uiModule.showToast(`Using ${m.display}`);
  }
  ...
  document.addEventListener('odysseus:auto-select-model', async (e) => {
    ...
    // Give admin/shared defaults first refusal. Without this await, the
    // no-detail auto-select path can grab the first cached model before
    // /api/default-chat resolves for secondary accounts.
    await _ensureDefaultPendingChat({ allowFallback: false });
    const currentAfterDefault = _deps.getCurrentSessionId();
    const currentDefault = sessions.find(x => x.id === currentAfterDefault);
    const pendingDefault = _deps.getPendingChat();
    if ((currentDefault && currentDefault.model) || (pendingDefault && pendingDefault.modelId)) return;
  ...
  btn.addEventListener('click', (e) => {
    ...
    if (menu.classList.contains('hidden') || menu.classList.contains('closing')) {
      ...
      // Kick off a local-endpoint probe â€” when it returns, re-render
 ...
 * Always visible â€” shows current model name or "Select model" if none.
...
export function updateModelPicker() {
  ...
  // Check if selected model is still available â€” fall back ONLY for pending chats with no user selection
  // Never override an existing session's model â€” the user explicitly chose it
  if (modelId && !currentSessionId && _pendingChat && window.modelsModule && window.modelsModule.getCachedItems) {
    ...
    if (allAvailable.length > 0 && !allAvailable.includes(modelId)) {
      // Model no longer available â€” switch to first available
  ...
  label.title = modelId || '';
  const tierInfo = modelId ? _chatGptTierInfo(modelId) : null;
  const logo = tierInfo ? tierInfo.emoji : (modelId ? providerLogo(modelId) : null);
  if (logo) {
    label.innerHTML = '<span class="model-picker-logo ' + _modelProviderClass(modelId) + (tierInfo ? ' model-tier-logo model-tier-' + tierInfo.cls + '-logo' : '') + '">' + logo + '</span> ' + (tierInfo ? tierInfo.display : displayName);
  } else {
