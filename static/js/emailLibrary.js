...
export function openEmailLibrary(opts = {}) {
  ...
  const _composeNew = () => {
    ...
    // CyberVault: opening compose should replace the Email library, not sit
    // behind it. The old desktop split path kept the library visible, but this
    // dark modal theme makes that layer cover the new composer.
    closeEmailLibrary();
...
// the native <select> so each row can carry a Font Awesome icon. The hidden
...
const _EMAIL_FILTER_ICONS = {
  'all':           { cls: 'fa-layer-group', color: '#5eead4' },
  'unread':        { cls: 'fa-envelope-open-text', color: '#38bdf8' },
  'favorites':     { cls: 'fa-star', color: '#fbbf24' },
  'undone':        { cls: 'fa-square-check', color: '#a78bfa' },
  'reminders':     { cls: 'fa-bell', color: '#fb923c' },
  'unanswered':    { cls: 'fa-reply', color: '#60a5fa' },
  'pending_30d':   { cls: 'fa-clock', color: '#2dd4bf' },
  'stale_30d':     { cls: 'fa-calendar-xmark', color: '#f87171' },
  'tag:urgent':    { cls: 'fa-fire-flame-curved', color: '#ef4444' },
  'tag:reply-soon':{ cls: 'fa-reply-all', color: '#818cf8' },
  'tag:spam':      { cls: 'fa-ban', color: '#f97316' },
  'tag:newsletter':{ cls: 'fa-newspaper', color: '#22d3ee' },
  'tag:marketing': { cls: 'fa-bullhorn', color: '#e879f9' },
};

function _filterIcon(value) {
  const meta = _EMAIL_FILTER_ICONS[value] || _EMAIL_FILTER_ICONS['all'];
  return `<i class="fa-solid ${meta.cls}" aria-hidden="true" style="--email-filter-icon-color:${meta.color}"></i>`;
}

function _filterIconColor(value) {
  const meta = _EMAIL_FILTER_ICONS[value] || _EMAIL_FILTER_ICONS['all'];
  return meta.color;
}
...
function _renderFilterPickerCurrent() {
  ...
  document.querySelectorAll('#email-filter-menu .email-filter-item').forEach(item => {
...
function _initFilterPicker() {
  ...
  menu.innerHTML = items.map(it => {
    ...
    return `<button type="button" role="option" class="email-filter-item" data-value="${it.value}" style="--email-filter-icon-color:${_filterIconColor(it.value)}">
