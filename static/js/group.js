...
let _parentPersistChain = Promise.resolve();
...
export async function startGroup(models, parentSessionId) {
  ...
  for (const m of models) {
    try {
      ...
      let sysPrompt;
      if (m.character) {
        const characterPrompt = String(m.character.characterPrompt || '').trim();
        sysPrompt = [
          `Your name is ${displayName}.`,
          characterPrompt,
          `You're in a group discussion with ${otherNames} and the user. ${_groupEtiquette} Stay in character.`,
        ].filter(Boolean).join('\n\n');
      } else {
        sysPrompt = `You are ${displayName} in a group chat with ${otherNames} and the user. ` +
          _groupEtiquette;
      }
      m._groupSystemPrompt = sysPrompt;
...
export async function sendMessage(msg) {
  ...
  // Persist the user turn before model replies so reload restores the same order.
  await _persistParentMessage('user', msg);
...
async function _sendParallel(msg, box) {
  ...
  await _persistGroupResponses(holders);
...
async function _sendRoundRobin(msg, box) {
  ...
  for (let turn = 0; turn < order.length; turn++) {
    ...
    if (response) {
      await _persistParentMessage('assistant', response, _groupMetadataFor(m, idx));
    }
    if (response) {
...
function _groupMetadataFor(model, idx) {
  const gName = model?._groupName || model?.character?.characterName || model?.display || `Participant ${idx + 1}`;
  return { group_model: gName, model: model?.mid || '' };
}

async function _persistGroupResponses(holders) {
  for (let i = 0; i < holders.length; i++) {
    const response = holders[i]?.dataset?.raw || '';
    if (!response) continue;
    await _persistParentMessage('assistant', response, _groupMetadataFor(_models[i], i));
  }
}

function _persistParentMessage(role, content, metadata) {
  if (!_parentSessionId || !content) return Promise.resolve();
  const sid = _parentSessionId;
  const msg = { role, content };
  if (metadata && Object.keys(metadata).length) msg.metadata = metadata;
  const save = async () => {
    const res = await fetch(`${API_BASE}/api/session/${sid}/inject_messages`, {
      method: 'POST',
      credentials: 'same-origin',
      headers: { 'Content-Type': 'application/json' },
  ...
  _parentPersistChain = _parentPersistChain.then(save, save).catch((e) => {
    console.warn('[group] parent persistence failed:', e);
  });
  return _parentPersistChain;
}
...
async function _streamToHolder(modelIdx, sessionId, msg, holderEl, abortCtrl) {
  ...
  const groupSystemPrompt = _models[modelIdx]?._groupSystemPrompt || '';
  if (groupSystemPrompt) fd.append('group_system_prompt', groupSystemPrompt);
  const groupCharacterName = _models[modelIdx]?._groupName || _models[modelIdx]?.character?.characterName || '';
  if (groupCharacterName) fd.append('group_character_name', groupCharacterName);
