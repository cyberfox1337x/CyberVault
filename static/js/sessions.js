...
function _renderSessionListImpl() {
  ...
  const savedOrder = Storage.get('session-order');
  let orderedSessions = sessions.filter(s => !s.archived && s.folder !== 'Assistant' && !_isIncognitoSession(s.id) && (s.name || '').trim() !== 'Nobody' && (s.name || '').trim() !== 'Anonymous' && (s.name || '').trim() !== 'Incognito');
...
export async function loadSessions() {
  try {
    ...
    const rootUrlWantsBlank = !hashId && !currentSessionId && !hasPendingChat;
    if (rootUrlWantsBlank) {
      Storage.remove('lastSessionId');
      savedId = null;
    }
    let targetId = null;
    ...
    if (!targetId && !hasPendingChat) {
      ...
      if (!rootUrlWantsBlank && activeSessions.length === 0 && !_autoCreateInProgress) {
...
export function createDirectChat(url, modelId, endpointId) {
  _sessionNavToken++;
  if (window._cybervaultReturnToChatRoute) {
...
export async function materializePendingSession() {
  ...
  const name = isIncognito ? 'Anonymous' : `${base} ${new Date().toLocaleTimeString()}`;
...
export function openLibrary(defaultTab) {
    ...
    <div class="modal-content doclib-modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="memory-bulk-bar hidden" id="lib-bulk-bar">
          ...
          <button class="memory-toolbar-btn" id="lib-bulk-action1">Archive</button>
  ...
  document.getElementById('lib-close').addEventListener('click', (e) => {
    e.preventDefault();
    e.stopPropagation();
...
export function closeLibrary(opts = {}) {
  const modal = document.getElementById('library-modal') || document.getElementById('doclib-modal');
  const shouldReturnToChatRoute = !!(
    opts &&
    opts.returnToChatRoute &&
    (modal?.classList.contains('cybervault-route-view') || window.location.pathname === '/documents' || window.location.pathname === '/library')
  );
  if (modal) {
