...
function _admProviderKeyForOption(opt) {
  if (!opt) return 'custom';
  const raw = opt.dataset?.providerKey || opt.dataset?.logo || opt.value || opt.textContent || 'custom';
...
function initEndpointForm() {
  ...
  async function _startProviderDeviceAuth(providerKey, triggerEl = null, statusEl = null, options = {}) {
    if (deviceAuthPolling) return;
    const config = PROVIDER_DEVICE_FLOWS[providerKey];
    if (!config) return;
    const status = statusEl || el('adm-deviceAuthStatus') || _endpointMsg('api');
    if (!status) return;
    ...
    const showAuthError = (text) => {
      ...
      retry.addEventListener('click', () => { _startProviderDeviceAuth(providerKey, triggerEl, status, options); });
    ...
    try {
      const result = await runProviderDeviceFlow(providerKey, {
        openWindow: (url) => {
          if (!options.openAuthWindow || !url) return;
  ...
  const ADD_MODEL_SECTION_STATE_KEY = 'cybervault:add-model-section-state';
  function _loadAddModelSectionState() {
    try {
      const parsed = JSON.parse(localStorage.getItem(ADD_MODEL_SECTION_STATE_KEY) || '{}');
      return parsed && typeof parsed === 'object' ? parsed : {};
    } catch (_) {
      return {};
    }
  }
  function _saveAddModelSectionState(state) {
    try { localStorage.setItem(ADD_MODEL_SECTION_STATE_KEY, JSON.stringify(state || {})); } catch (_) {}
  }
  function _wireAddModelSectionToggle(btnId, sectionId, key) {
    const btn = el(btnId);
    const section = el(sectionId);
    if (!btn || !section || btn.dataset.bound === '1') return;
    btn.dataset.bound = '1';
    const state = _loadAddModelSectionState();
    const apply = (open) => {
      section.classList.toggle('collapsed', !open);
      btn.classList.toggle('collapsed', !open);
      btn.setAttribute('aria-expanded', open ? 'true' : 'false');
      btn.title = (open ? 'Collapse ' : 'Expand ') + (btnId.includes('Local') ? 'Add Local Models' : btnId.includes('Api') ? 'Add API Models' : 'Add OAuth Models');
    };
    ...
    btn.addEventListener('click', (e) => {
      ...
      const nextState = _loadAddModelSectionState();
      nextState[key] = nextOpen;
      _saveAddModelSectionState(nextState);
      apply(nextOpen);
    });
  }
  _wireAddModelSectionToggle('adm-epLocalCollapseBtn', 'adm-epLocalSection', 'local');
  _wireAddModelSectionToggle('adm-epApiCollapseBtn', 'adm-epApiSection', 'api');
  _wireAddModelSectionToggle('adm-epOAuthCollapseBtn', 'adm-epOAuthSection', 'oauth');

  function _wireOAuthModelPicker() {
    const select = el('adm-oauthProvider');
    const picker = el('adm-oauth-provider-picker');
    const btn = el('adm-oauth-provider-btn');
    const menu = el('adm-oauth-provider-menu');
    const current = picker ? picker.querySelector('.adm-provider-current') : null;
    if (!select || !picker || !btn || !menu || !current || picker.dataset.bound === '1') return;
    picker.dataset.bound = '1';
    const logoFor = (opt) => providerLogo((opt && (opt.dataset.logo || opt.textContent || opt.value)) || '') || '';
    const render = () => {
      menu.innerHTML = Array.from(select.options).map((opt) => {
        const active = opt.value === select.value ? ' active' : '';
        const key = _admProviderKeyForOption(opt);
        return '<div class="adm-provider-item' + active + '" role="option" data-value="' + esc(opt.value) + '" data-provider-key="' + esc(key) + '">' +
          '<span class="adm-provider-logo" data-provider-key="' + esc(key) + '">' + logoFor(opt) + '</span>' +
          '<span class="adm-provider-name">' + esc(opt.textContent || '') + '</span>' +
        '</div>';
      }).join('');
    };
    const sync = () => {
      ...
      current.dataset.providerKey = key;
      const logo = current.querySelector('.adm-provider-logo');
      const name = current.querySelector('.adm-provider-name');
      if (logo) {
        logo.dataset.providerKey = key;
        logo.innerHTML = logoFor(opt);
      }
      if (name) name.textContent = (opt && opt.textContent) || 'OAuth provider';
      render();
    };
    btn.addEventListener('click', (e) => {
      e.preventDefault();
  ...
  _wireOAuthModelPicker();

  const oauthSignInBtn = el('adm-oauthSignInBtn');
  if (oauthSignInBtn && oauthSignInBtn.dataset.bound !== '1') {
    oauthSignInBtn.dataset.bound = '1';
    oauthSignInBtn.setAttribute('type', 'button');
    oauthSignInBtn.addEventListener('click', async (e) => {
      e.preventDefault();
      e.stopPropagation();
      e.stopImmediatePropagation();
      const select = el('adm-oauthProvider');
      const status = el('adm-oauthStatus');
      const providerKey = select ? select.value : '';
      if (status) {
        status.textContent = '';
        status.className = 'adm-ep-inline-msg';
      }
      if (providerKey === 'chatgpt-subscription') {
        await _startProviderDeviceAuth('chatgpt-subscription', oauthSignInBtn, status, { openAuthWindow: true });
        return;
      }
      if (providerKey === 'claude-oauth') {
        window.open('https://claude.ai/login', '_blank', 'noopener');
        if (status) {
          status.className = 'adm-ep-inline-msg adm-oauth-note';
          status.textContent = 'Claude sign-in opened. CyberVault does not have a Claude OAuth token-exchange route yet, so this cannot add Claude models automatically until backend support is added.';
        }
      }
    });
  }
