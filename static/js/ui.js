...
function _pointerOverInteractivePreviewFrame(surface) {
  if (_lastPointerClientX == null || _lastPointerClientY == null) return false;
  const frames = document.querySelectorAll('iframe.doc-html-preview, iframe.compare-pane-iframe');
  for (const frame of frames) {
    if (surface && !surface.contains(frame)) continue;
    if (!_isSpaceVisible(frame) || getComputedStyle(frame).display === 'none') continue;
    if (_containsPointer(frame)) return true;
  }
  return false;
}
...
function _spaceIsBlocked(e, surface) {
  ...
  // The hover-space shortcuts operate from pointer state, while keydown targets
  // the element that currently owns keyboard focus. If the user is typing in an
  // editor anywhere on the page, Space must stay with that editor instead of
  // activating/minimizing whatever the mouse happens to be hovering.
  if (_isTextEditingTarget(target) || _isTextEditingTarget(document.activeElement)) return true;
  if (_pointerOverInteractivePreviewFrame(surface)) return true;
