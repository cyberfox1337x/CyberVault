 ...
 */
function _looksLikeSingleDollarMath(math, match, source, offset) {
  const raw = String(math || '').replace(/&amp;/g, '&').replace(/&lt;/g, '<').replace(/&gt;/g, '>');
  ...
  if (/\d/.test(before) && !/[A-Za-z)]/.test(trimmed[0] || '')) return false;
  if (/^[+-]?(?:\d|[.,]\d)/.test(trimmed)) return false;
  if (/\b(?:to|through|thru|and|or)\s*$/i.test(trimmed)) return false;

  return /\\[A-Za-z]+|[=^_{}]|(?:[A-Za-z0-9)\]}]\s*[+\-*/]\s*[A-Za-z0-9({[])/.test(trimmed);
}
...
export function mdToHtml(src, opts) {
  ...
  if (window.katex) {
    ...
    // Inline math: $...$ is ambiguous with currency, so only render spans
    // that look like real math. Use \( ... \) when prose needs certainty.
    s = s.replace(/(?<!\$)\$(?!\$)([^\$\n]+?)\$(?!\$)/g, (match, math, offset, source) => {
      if (!_looksLikeSingleDollarMath(math, match, source, offset)) return match;
      try {
        const raw = math.replace(/&amp;/g, '&').replace(/&lt;/g, '<').replace(/&gt;/g, '>');
        const placeholder = `___MATH_BLOCK_${mathBlocks.length}___`;
        mathBlocks.push(katex.renderToString(raw.trim(), { displayMode: false, throwOnError: false }));
