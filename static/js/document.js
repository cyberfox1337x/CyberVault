...
import { bindMenuDismiss, dismissOrRemove } from './escMenuStack.js';
  ...
  const _highlightElementSafe = (el) => {
    ...
    try {
      ...
      console.warn('Skipping unsupported document highlight language', lang || '(auto)', err);
    }
  };
  ...
  export function openPanel() {
    ...
    pane.innerHTML = `
      ...
      <iframe id="doc-html-preview" class="doc-html-preview" sandbox="allow-scripts allow-modals" tabindex="0" title="Document HTML preview" style="display:none"></iframe>
    ...
    if (ta && pre) {
      ...
      ta.addEventListener('scroll', () => {
        ...
        if (_findMatches && _findMatches.length) {
          ...
          if (_q) {
            const ranges = _findRangesForRender ? _findRangesForRender(_q) : _findMatches.map(s => [s, s + _q.length]);
            renderFindRects(ranges, ranges.length === 1 ? 0 : _findIdx);
          }
        }
      });
      ...
      const _FIND_MARK_LIMIT = 500;
      const _FIND_RECT_LIMIT = 250;

      function _findRangesForRender(q) {
        if (!_findMatches || !_findMatches.length || !q) return [];
        if (_findMatches.length > _FIND_RECT_LIMIT && _findIdx >= 0) {
          const s = _findMatches[_findIdx];
          return [[s, s + q.length]];
        }
        return _findMatches.map(s => [s, s + q.length]);
      }

      function _applyFindVisuals(codeEl, q) {
        if (!codeEl) return;
        if (_findMatches.length <= _FIND_MARK_LIMIT) {
      ...
      document.getElementById('doc-find-input')?.addEventListener('input', () => {
        ...
        _findInputTimer = setTimeout(() => _doFind('first', false), 120);
      });
  ...
  function _setMarkdownPreviewActive(active, { remember = true } = {}) {
    ...
    if (active) {
      ...
      if (window.hljs) {
        preview.querySelectorAll('pre code').forEach(b => _highlightElementSafe(b));
      }
  ...
  function _focusHtmlPreviewFrame(iframe) {
    if (!iframe) return;
    iframe.setAttribute('tabindex', '0');
    const focusFrame = () => {
      requestAnimationFrame(() => {
        try { iframe.focus({ preventScroll: true }); } catch (_) { try { iframe.focus(); } catch (_) {} }
        try { iframe.contentWindow && iframe.contentWindow.focus(); } catch (_) {}
      });
    };
    iframe.addEventListener('load', focusFrame, { once: true });
    setTimeout(focusFrame, 0);
    setTimeout(focusFrame, 80);
  }
