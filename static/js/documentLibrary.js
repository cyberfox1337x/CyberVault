...
const _hljsLanguageFor = (lang) => {
  const normalized = (lang || '').toLowerCase();
  if (!normalized || normalized === 'text' || normalized === 'txt' || normalized === 'markdown' || normalized === 'email') return '';
  const mapped = normalized === 'svg' ? 'xml' : normalized;
  if (window.hljs?.getLanguage && !window.hljs.getLanguage(mapped)) return '';
  return mapped;
};
...
// ---- Library animation helpers ----
  ...
  function libraryCreateCard(doc) {
    ...
    try {
      const hlLang = _hljsLanguageFor(doc.language);
      if (hlLang && window.hljs && !_librarySearch) {
        code.innerHTML = window.hljs.highlight(doc.preview || '', { language: hlLang }).value;
  ...
  async function libraryExpandCard(card, doc) {
    ...
    try {
      ...
      const HL_CAP = 20000;
      try {
        const hlLang = _hljsLanguageFor(lang);
        if (hlLang && window.hljs && content.length <= HL_CAP) {
          code.innerHTML = window.hljs.highlight(content, { language: hlLang }).value;
        } else {
  ...
  export function openLibrary(opts) {
    ...
    // Wire events
    const doclibCloseBtn = document.getElementById('doclib-close');
    const closeDoclibRouteView = (e) => {
      e.preventDefault();
      e.stopPropagation();
      if (typeof e.stopImmediatePropagation === 'function') e.stopImmediatePropagation();
      closeLibrary({ returnToChatRoute: true });
    };
    doclibCloseBtn.addEventListener('pointerdown', closeDoclibRouteView, true);
    doclibCloseBtn.addEventListener('click', closeDoclibRouteView, true);
  ...
  export function closeLibrary(opts = {}) {
    const modal = document.getElementById('doclib-modal');
    if (!_libraryOpen && !modal) return;
    const shouldReturnToChatRoute = !!(
      opts &&
      opts.returnToChatRoute &&
      (modal?.classList.contains('cybervault-route-view') || window.location.pathname === '/documents')
    );
    _libraryOpen = false;
    _librarySelectMode = false;
    _librarySelectedIds.clear();
    ...
    if (shouldReturnToChatRoute) {
      setTimeout(() => {
        try {
          if (typeof window._cybervaultReturnToChatRoute === 'function') {
            window._cybervaultReturnToChatRoute();
          } else if (typeof window._cybervaultOpenRoutePath === 'function') {
            window._cybervaultOpenRoutePath('/');
          } else if (window.location.pathname !== '/') {
            window.history.pushState({ cybervaultRoute: '/' }, '', '/');
            document.body.dataset.cybervaultRoute = '/';
            document.body.classList.remove('cybervault-dashboard-route');
            document.body.classList.add('cybervault-chat-route');
            window.dispatchEvent(new CustomEvent('cybervault:route-change', { detail: { route: '/' } }));
          }
        } catch (_) {}
      }, 0);
    }
